...
///   project/{goal,context,state,backlog,decisions,learnings,constraints,resources,metrics,CONTEXT_BUDGET}.md
...
public static class ScaffoldGenerator
{
    ...
    /// </summary>
    public static readonly string[] ProjectSubdirs = ProjectComponentCatalog.Components.Select(c => c.Id).ToArray();
    ...
    public static void Generate(string root, Requirements req, Architecture arch)
    {
        ...
        var included = decisions.Where(d => d.Included).Select(d => d.Id).ToHashSet();
        ...
        WriteMd(root, "README.md", "readme",
            "Human-facing project overview", ReadmeMd(req, arch, decisions));
        ...
        WriteMd(root, Path.Combine("project", "CONTEXT_BUDGET.md"), "context-budget",
            "Conventions and optional integrations for keeping token usage low", ContextBudgetMd(req));
...
""";
    ...
    private static string ContextBudgetMd(Requirements req) => $"""
# Context budget
...
to plug in more if this project's budget profile ({req.BudgetProfile})
needs it.
