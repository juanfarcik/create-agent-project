# software-project — Agent Instructions

This is a **software** project. Architecture: **software-standard**.
...
- `architect` (on-demand, strong) — Defines technical structure, technology choices, and tradeoffs before implementation.
- `coder` (always, balanced) — Implements the code required to satisfy a task, following the current design.
- `tester` (always, cheap) — Writes and runs automated tests, and reports failures with a concrete repro.
- `code-reviewer` (always, balanced) — Reviews code changes for correctness, security, and unnecessary complexity before merge.
- `bi-analyst` (on-demand, cheap) — Analyzes product/usage data and metrics to inform decisions, on demand.
