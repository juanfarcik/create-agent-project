...
# business-research — Agent Instructions

This is a **business** project. Architecture: **collaborative**.
...
- `orchestrator` (always, strong) — Coordinates the project and decides the next highest-value action.
- `researcher` (always, cheap) — Reduces uncertainty by gathering evidence and comparing alternatives.
- `analyst` (always, balanced) — Transforms information into conclusions, separating observation from inference.
- `critic` (on-demand, balanced) — Challenges assumptions and identifies weaknesses, on demand.
